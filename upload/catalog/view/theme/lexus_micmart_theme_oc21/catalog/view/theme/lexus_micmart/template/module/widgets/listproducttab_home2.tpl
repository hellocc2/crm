		...
		<div class="clearfix <?php echo $addition_cls; ?> panel-v5">
			...
			<div class="pull-right">
				<ul class="nav nav-tabs" role="tablist">
					<?php foreach ($categories as $k=>$link): ?>
						<li class="<?php if($k==0) {?>active<?php } ?>"><a href="#tab-<?php echo $link['category_id']; ?>" data-toggle="tab"><?php echo $link['name']; ?></a></li>
					<?php endforeach ?>
				</ul>
			</div>
		</div>
	<div class="tab-content col-nopadding">
		<?php $i=0; foreach ($arr_category as $k => $value): ?>
			<div class="tab-pane <?php if($i==0) {?>active<?php } ?>" id="tab-<?php echo $k; ?>">
				<?php 
					$first_product = $value['first_product'];
					$list1 = $value['list_product'];
				?>
				<div class="row">
					<!-- start div row -->
					<?php if(!empty($first_product)) { ?>
					<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4 product-v3 border-right">
						<?php require( DIR_TEMPLATE.$config->get('config_template').'/template/common/product/first_product.tpl');?>
					</div>
					<?php } ?>

					<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8 effect-carousel-v2 product-v4">	
						<div class="list box-products slide" id="carousel<?php echo $id . $k;?>">
							<?php if( count($list1) > $itemsperpage ) { ?>
								<div class="carousel-controls-v1">
									<a class="carousel-control carousel-sm left" href="#carousel<?php echo $id . $k;?>"   data-slide="prev">
										<i class="fa fa-angle-left"></i>
									</a>
									<a class="carousel-control carousel-sm right" href="#carousel<?php echo $id . $k;?>"  data-slide="next">
...
<div class="listproduct-v2">
	<div class="clearfix <?php echo $addition_cls; ?> panel-v5">
		...
		<div class="pull-right">
			...
			<ul class="links space-padding-r20 space-padding-t30">
				...
				<li><a href="<?php echo $category["href"] ?>"><i class="fa fa-plus"></i><?php echo $category["name"] ?></a></li>
				<?php endforeach ?>
			</ul>
			<?php endif ?>
		</div>
   ...
   <div class="bg-white col-nopadding">
	<div class="row">
		...
		<!-- col2 first product -->
		<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 product-v3 border-right">
			<?php require( DIR_TEMPLATE.$config->get('config_template').'/template/common/product/first_product.tpl');?>
		</div>

		<!-- column 1 -->		
		<div class="col-xs-5 col-sm-5 col-md-5 col-lg-5 effect-carousel-v2">	
