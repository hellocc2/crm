...
class ControllerModulePavsliderlayer extends Controller {
	...
	protected function preload(){
		$this->load->language('module/pavsliderlayer');
	...
	public function module($extension){
		...
		foreach ($modules as $module) {
			$module_data[] = array(
				...
				'delete'    => $this->url->link('module/pavsliderlayer/delete', 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], 'SSL')
			);
		}
	...
	public function index() {   
			...
			if( $this->request->post['action_mode'] == 'module-only' ) {
				...
				unset($this->request->post['action_mode']);
			 	...
			 	if (!isset($this->request->get['module_id'])) {
					...
					$this->response->redirect($this->url->link('module/pavsliderlayer', 'token=' . $this->session->data['token'], 'SSL'));
				} else {
					$this->model_extension_module->editModule($this->request->get['module_id'], $this->request->post);
					$this->response->redirect($this->url->link('module/pavsliderlayer', 'token=' . $this->session->data['token'].'&module_id='.$this->request->get['module_id'], 'SSL'));
				}
				...
				$this->response->redirect($this->url->link('module/pavsliderlayer', $id.'token=' . $this->session->data['token'], 'SSL'));
			}
		...
		$this->mdata['mdelete'] = $this->url->link('module/pavsliderlayer/mdelete', 'token=' . $this->session->data['token'].$url, 'SSL');
		$this->mdata['action'] = $this->url->link('module/pavsliderlayer', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		$this->response->setOutput($this->load->view('module/sliderlayer/sliders.tpl', $this->mdata));
	}
	...
	public function layer(){
		...
		$this->mdata['action'] = $this->url->link('module/pavsliderlayer/savedata', 'token=' . $this->session->data['token'], 'SSL');
		$this->mdata['cancel'] = $this->url->link('module/pavsliderlayer', 'token=' . $this->session->data['token'], 'SSL');		
		$this->mdata['token'] = $this->session->data['token'];
		$this->mdata['actionUpdatePostURL'] = $this->url->link('module/pavsliderlayer/savepos', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('module/sliderlayer/layer.tpl', $this->mdata));
	}
	...
	public function copythis(){
		if (!$this->user->hasPermission('modify', 'module/pavsliderlayer')) {
	 	...
	 	if( isset($this->request->get['id']) ){
	 		...
	 		$url = $this->url->link('module/pavsliderlayer/layer', 'id='.$id.'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $this->session->data['token'], 'SSL');
	...
	public function cloneGroupSliders(){
 	...
 	public function deleteslider(){
		...
		$url = $this->url->link('module/pavsliderlayer/layer', 'group_id='.$group_id.'&lang='.$lang.'&token=' . $this->session->data['token'], 'SSL');
	 	$this->response->redirect( $url );
 	}
	...
	public function previewLayer() {
		...
		$template = $this->load->view('module/sliderlayer/preview.tpl', $this->mdata);
	...
	public function previewGroup() {	
		...
		$template = $this->load->view('module/sliderlayer/previewgroup.tpl', $this->mdata);
		$this->response->setOutput($template);
	...
	public function typo(){
		...
		$data = preg_match_all("#\.iview-caption\.(\w+)\s*{\s*#", $content, $matches);
		...
		$template = $this->load->view('module/sliderlayer/typo.tpl', $this->mdata);
	...
	public function filemanager() {
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('module/pavsliderlayer/filemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url, 'SSL')
				);
		...
		$data['parent'] = $this->url->link('module/pavsliderlayer/filemanager', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$data['refresh'] = $this->url->link('module/pavsliderlayer/filemanager', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$pagination->limit = 16;
		$pagination->url = $this->url->link('module/pavsliderlayer/filemanager', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$this->response->setOutput($this->load->view('module/sliderlayer/filemanager.tpl', $data));
	}
